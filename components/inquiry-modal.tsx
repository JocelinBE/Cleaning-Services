...
const MODAL_BG = "hsl(200, 32%, 11%)"
const MODAL_BG_TOP = "hsl(200, 28%, 14%)"
const ACCENT = "hsl(200, 75%, 68%)"
const ACCENT_SOFT = "hsl(200, 60%, 72%)"

const WHATSAPP_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "16195550123"
const WEB3FORMS_ACCESS_KEY = process.env.NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY || ""

const inputBase =
  "w-full px-4 py-3.5 border border-white/15 bg-white/[0.06] text-white placeholder:text-white/35 focus:outline-none focus:ring-2 focus:ring-[hsl(200,75%,68%)]/40 focus:border-[hsl(200,75%,68%)] transition-all duration-200 text-sm rounded-xl"
...
export function InquiryModal({ isOpen, onClose }: InquiryModalProps) {
  ...
  // GSAP Draggable (igual que viewtransition): handle en header, inercia, escala al bajar, cierre o snap
  useEffect(() => {
    if (!isOpen || !panelRef.current || !headerRef.current) {
    ...
    const maxX = (windowWidth - modalWidth) / 2 + 100
    const maxY = windowHeight + 200
    const isMobile = window.innerWidth < 768 || "ontouchstart" in window
    const threshold = isMobile ? Math.min(80, windowHeight * 0.12) : DRAG_CLOSE_THRESHOLD

    gsap.set(panel, { x: 0, y: 0, scale: 1, opacity: 0, transformOrigin: "center center" })

    const setupDraggable = () => {
      const draggable = Draggable.create(panel, {
      type: "x,y",
      handle: header,
      bounds: { minX: -maxX, maxX, minY: -50, maxY },
      inertia: true,
      dragResistance: isMobile ? 0.1 : 0.5,
      throwResistance: isMobile ? 300 : 1000,
      force3D: true,
      onDrag: function () {
        if (this.y > 0) {
          const scale = Math.max(0.7, 1 - this.y / 400)
      ...
      const t = setTimeout(() => {
        if (draggableInstanceRef.current) draggableInstanceRef.current[0].update()
      }, 100)

      const handleResize = () => {
        if (!draggableInstanceRef.current || !panel) return
        const w = window.innerWidth
        const h = window.innerHeight
        const r = panel.getBoundingClientRect()
        const mx = (w - r.width) / 2 + 100
        draggableInstanceRef.current[0].vars.bounds = { minX: -mx, maxX: mx, minY: -50, maxY: h + 200 }
    ...
    return () => {
      draggableCleanupRef.current?.()
      draggableCleanupRef.current = null
      ctx.revert()
    }
  }, [isOpen, onClose])
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-hidden backdrop-blur-[6px]"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ref={panelRef}
        className="inquiry-modal-panel w-full max-w-xl max-h-[88vh] overflow-hidden flex flex-col rounded-2xl border border-white/[0.08]"
        style={{
      ...
      >
        ...
        <div
          ref={headerRef}
          className="modal-header-container flex-shrink-0 flex items-start justify-between gap-4 px-6 sm:px-8 pt-6 sm:pt-8 pb-5 border-b border-white/[0.08]"
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-3">
              <span className="text-[10px] tracking-[0.32em] uppercase text-white/45 font-medium">Get in touch</span>
              <div className="h-px flex-1 bg-gradient-to-r from-white/15 to-transparent" />
            </div>
            <h2 id="inquiry-modal-title" className="text-xl sm:text-2xl font-bold tracking-tight" style={{ color: ACCENT, fontFamily: "var(--font-playfair), serif" }}>
              You&apos;re one step away.
            </h2>
            <p className="text-sm text-white/60 mt-1.5">Choose how you&apos;d like us to reply—<strong className="text-white/80">email</strong> or <strong className="text-white/80">WhatsApp</strong>. We answer within 24 hours.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 active:scale-95 transition-all duration-200"
            aria-label="Close"
          >
        ...
        <div
          className="inquiry-modal-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-contain px-6 sm:px-8 py-5 sm:py-6"
          ...
          >
              ...
              <div className="space-y-5 py-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/[0.04] text-white/70 hover:border-white/25 hover:bg-white/[0.06] py-4 px-5 transition-all duration-200 text-sm font-medium"
                  >
            ...
            <Step>
              <div className="space-y-5 py-2">
                ...
                <div className="space-y-4">
                  <div>
                    <label htmlFor="inquiry-name" className="block text-xs font-medium mb-2 tracking-wide text-white/80">
                  ...
                  {formData.preferredContact === "email" && (
                    <>
                      <div>
                        <label htmlFor="inquiry-email" className="block text-xs font-medium mb-2 tracking-wide text-white/80">
                      ...
                      <div>
                        <label htmlFor="inquiry-phone" className="block text-xs font-medium mb-2 tracking-wide text-white/80">
            ...
            <Step>
              <div className="space-y-5 py-2">
                ...
                <div>
                  <label htmlFor="inquiry-message" className="block text-xs font-medium mb-2 tracking-wide text-white/80">
                    Message <span style={{ color: ACCENT }}>*</span>
                    <span className="text-white/50 ml-2 font-normal">({formData.message.length}/1000)</span>
                  </label>
                  <textarea
                    id="inquiry-message"
                    name="message"
                ...
                <div>
                  <label htmlFor="inquiry-schedule" className="block text-xs font-medium mb-2 tracking-wide text-white/70">
              ...
              <div className="space-y-5 py-2">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={field.label} className="rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3">
                      <div className="text-[10px] tracking-[0.2em] uppercase text-white/45 mb-1.5">{field.label}</div>
                      <div className="text-sm text-white/95 font-medium whitespace-pre-wrap leading-relaxed">{field.value || "—"}</div>
                    </div>
                  ))}
                </div>
                <p className="text-xs text-white/55 pt-1">We&apos;ll reply by {formData.preferredContact === "whatsapp" ? "WhatsApp" : "email"} within 24 hours.</p>
                {isSubmitting && (
